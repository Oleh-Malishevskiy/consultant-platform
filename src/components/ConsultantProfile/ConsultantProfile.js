...
function ProfileForm({ authUser }) {
    ...
    useEffect(() => {
        if (authUser && authUser.uid) {
            const userRef = doc(db, "users", authUser.uid); // assuming user role is stored in the users collection
            getDoc(userRef).then(docSnap => {
                if (docSnap.exists()) {
                    const userRole = docSnap.data().role; // fetching role from user's document
    ...
    return (
        <div>
            <form onSubmit={handleSubmit} className="profile-form">
                ...
                <button type="button" onClick={handleDelete} style={{ backgroundColor: 'red', marginLeft: '10px' }}>Delete Profile</button>
            </form>
        </div>
    );
}
